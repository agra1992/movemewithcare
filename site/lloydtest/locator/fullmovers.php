...
function check_zipcode($zipcode, $or_state)
{
...
check_zipcode(& $zipcode, $or_state);
	$ph1 = $_POST[ph1];
	$ph2 = $_POST[ph2];
	$ph3 = $_POST[ph3];
	$ph4 = $_POST[ph4];
...
function  get_movers($type, $members ,$zipcode, $or_state, $or_shname)
{
    $proace="";
    $country="";
    if($or_state<52){
...
$database ="";
    if($country == "canada")
    {
        $sql = "SELECT `latitude`, `longitude` FROM zip_canada WHERE zipcode='$zipcode' LIMIT 1";
        $r = mysql_query($sql) or die("Failed Query:122 ");
        list($lat, $long) = mysql_fetch_array($r);
        $zip_database="zip_canada";
    }else{
        $sql = "SELECT `lat`, `lon` FROM zip_usa WHERE zipcode='$zipcode' LIMIT 1";
        $r = mysql_query($sql) or die("Failed Query:123 $sql");
        list($lat, $long) = mysql_fetch_array($r);
        $zip_database="zip_usa";      
		 	...
		 	if ($type=="long_distance")
			 	...
			 	$query10 = "Select tblmembers.MemberID,tblmembers.MemberName,tblmembers.Associations,CONCAT(SUBSTR(tblmembers.Description,1,300),'...') as Description,
		           	tblmembers.Logo, tblmembers.TollFree, tblmembers.ContactEmail,  tblmembers.USDot, tblmembers.MC, tblmembers.sms_service, tblmembers.sms_address, IFNULL(ROUND(DEGREES(ACOS(SIN(RADIANS($lat))*SIN(RADIANS(lat))+COS(RADIANS($lat))*COS(RADIANS(lat))*COS(RADIANS($long-lon)))),2)* 69.09,0) AS distance
		            From tblmembers LEFT JOIN $zip_database USING (zipcode) Where tblmembers.MemberType = 'full' AND tblmembers.Active = '1' AND tblmembers.InterstateLicense = '1' AND
						   ((tblmembers.State like 'NA'  && tblmembers.ServiceCountry='$country' && (IFNULL(ROUND(DEGREES(ACOS(SIN(RADIANS($lat))*SIN(RADIANS(lat))+COS(RADIANS($lat))*COS(RADIANS(lat))*COS(RADIANS($long-lon)))),2)* 69.09,0)  < tblmembers.distance)) || (tblmembers.MemberState='999' && tblmembers.ServiceCountry='$country')) ". $proace ."  Order by RAND()";
		 ...
		 for($i=0;$i<$num;$i++)
		 {
		 	array_push($members,mysql_fetch_assoc($result));
		 }		
		 		...
		 		$query10 = "Select tblmembers.MemberID,tblmembers.MemberName,tblmembers.Associations,CONCAT(SUBSTR(tblmembers.Description,1,300),'...') as Description,
	           	tblmembers.Logo, tblmembers.TollFree, tblmembers.ContactEmail,  tblmembers.USDot, tblmembers.MC, tblmembers.sms_service, tblmembers.sms_address, IFNULL(ROUND(DEGREES(ACOS(SIN(RADIANS($lat))*SIN(RADIANS(lat))+COS(RADIANS($lat))*COS(RADIANS(lat))*COS(RADIANS($long-lon)))),2)* 69.09,0) AS distance
	            From tblmembers LEFT JOIN $zip_database USING (zipcode) Where 
tblmembers.MemberType = 'full' AND 
tblmembers.Active = '1' AND				   
(
((IFNULL(ROUND(DEGREES(ACOS(SIN(RADIANS($lat))*SIN(RADIANS(lat))+COS(RADIANS($lat))*COS(RADIANS(lat))*COS(RADIANS($long-lon)))),2)* 69.09,0) < tblmembers.distance)
 AND(
         (tblmembers.State = '$or_shname')  OR 
         (tblmembers.State like 'NA')
     )) OR 
    (tblmembers.MemberState='999' && tblmembers.ServiceCountry='$country')
)  Order by RAND()";		
		 $result = mysql_query($query10) or die("Query failed235 $query10");
		 $members= array();
		 $num = mysql_num_rows($result);
		 for($i=0;$i<$num;$i++)
		 {
...
function store_leads($members, $LeadId)
{
    $values="";
    foreach($members as $member)
    {
        $values.="($LeadId , $member[MemberID] ),";
    }
    $values = substr($values, 0 , -1);
    $sql="INSERT INTO leads_sent (lead_id, member_id) VALUES $values";
    $r = mysql_query($sql);
...
//mail part
		 ...
		 $result = mysql_query($sql) or die("Query failed9");		              	 
...
$ret = array();
		 	if ($or_state != $dor_state)
                          {
                  get_movers("long_distance", & $ret, $zipcode,$or_state, $or_shname);
			 	
		 	}else {
                  get_movers("local_distance", & $ret , $zipcode,$or_state, $or_shname);
		 			} 	

		store_leads($ret, $LeadId);
		 ...
		 foreach($ret as $val)
		 {
		         $message  = str_replace ("%distance%", $val[distance], $message);	
...
$count++;  
			...
			$result = mysql_query($sql) or die("Query failed10");
...
Please log in to your panel for more details.
 Thank you ','$LeadId')";
		 $result = mysql_query($sql) or die("Query failed11");
                 }
		 }
...
$color='#EDFBEB';}
$which_color++;
    if($line[MemberState] == 999)
    {
        $line[distance]="N/A (this mover has sevaral offices around the country and can service you anywhere in the country)";
    } 
echo"
...
<tr >     
      ...
      <td width='400' class=regular_result_text>•Toll Free: ". $line[TollFree] ."<br>
       • Association: $assoc <br>
       • Description: ".$line[Description]."</td>
      <td class=license_info>
      •MC: ".$line[MC]."<br>
      •USDot: ".$line[USDot]."<br>
      •Distance between mover and you: ".$line[distance]." mi
        </td>
...
?>
 ...
 <table  border="0" cellspacing="0" cellpadding="0" name="top" id="tab_gray_text" id="main_frame">
   ...
   <td >&nbsp;   </td>
    ...
    <tr> 
      <td width="37%"><div align="right">Salutation:</div></td>
...
<br>
	...
	<tr> 
      <td><div align="right">Current City:<br />         <div id="cityrec">
	  <i>
...
<tr>
   <td><div align="right">Destination State/Province:</td>
 
<td>
 
      <select name="dor_state" size="1" id="dor_state" style="width: 170px; ">
            <option value="">Select State/Province</option>
<?
...
<br>
    ...
    <tr> 
  		...
  		<td><font  size="-1" face="Verdana,Arial, Helvetica, sans-serif">
	        ...
	        - <input name="ph3" type="text" id="ph3" size="4" maxlength="4" class="formobject" onKeyUp='Move(this,4,"ph3",event,"ph2")' <?if($autofill) {echo "value='$ph3'";}else if($ph3){echo"value='$ph3'";}?>></font></td>
    </tr>
 	<tr> 
        ...
        - <input name="ph6" type="text" id="ph6" size="4" maxlength="4" class="formobject" onKeyUp='Move(this,4,"ph6",event,"ph5")' <?if($autofill) {echo "value='$ph6'";}else if($ph6){echo"value='$ph6'";}?>></font></td>
	</tr>
    <tr> 
...
<input name="st_date" type="text" style="margin-left:0px;" alt="Estimate Date of Moving" id="st_date" class="formobject" size="10" maxlength="10" readonly onMouseOver="overlib('Please click on the calendar link to the right and choose a date from pop-up calendar.');return true;" onMouseOut="window.status=''; nd(); return true;" value="<? if($st_date){echo"$st_date";}?>">
...
<?php dsp_crypt(0,1); ?>
  ...
  <tr align="center"><td>&nbsp;</td><td align="left"><input type="submit" name="Submit" value="Request a Full Mover" id="next" STYLE="width: 220px; font-size: x-small; font-family: Arial; color: #; background-color: #dceffe; border: 1 outset #;margin-right:50px;"></td></tr>
<tr>
