...
function store_orders($or_ret, $dor_ret, $LeadId, $valid)
{

    $values="";
    foreach($or_ret as $member)
    {

        $values.="($LeadId , $member[0] , 'origin', '$valid'),";
    }
...
////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////
function check_zipcode($zipcode, $or_state,$dest_zipcode, $dor_state)
{
     ...
     if($or_state < 52){
        ...
        if(strlen($zipcode) == 6){
            $temp1 = substr(3, $zipcode);
            $temp2 = substr(-3, $zipcode);
            $zipcode = $temp1." ".$temp2;
        }
    }

     if($dor_state < 52){
        $dest_zipcode = (int)$dest_zipcode;
    }else{
        $dest_zipcode = strtoupper($dest_zipcode);

        $dest_zipcode = str_replace("-", " ", $dest_zipcode);
        if(strlen($dest_zipcode) == 6){
            $temp1 = substr(3, $dest_zipcode);
            $temp2 = substr(-3, $dest_zipcode);
            $dest_zipcode = $temp1." ".$temp2;
        }
...
function member_origin($LeadId, $or_service, $origin, $st_date, $labors, $distance)
{
...
$customer_info['address']=$address;
	...
	$zipcode = sanitize($_POST[zipcode],7,0);
$customer_info['zipcode']=$zipcode;

	$dest_zipcode = sanitize($_POST[dest_zipcode],7,0);
$customer_info['dest_zipcode']=$dest_zipcode;
...
if($dor_state==$or_state)
{
        ...
        check_zipcode(& $zipcode, $or_state,& $dest_zipcode, $dor_state);
...
{
    $sql = "INSERT INTO `tblcustomers` (Sal, FName, LName, Address, ZipCode, Dest_ZipCode, Phone, Phone2, email, DateAdded, IP)
		     VALUES ('$salut', '$fname', '$lname', '$address', '$zipcode','$dest_zipcode', $phone1, $phone2, '$email',CURRENT_TIMESTAMP, '$IP')"; 
...
{
    $q= "SELECT Valid, LastLupu, CustomerID from tblcustomers WHERE IP='$IP'";
    $r= mysql_query($q); 
...
if($work=="true"){
$query = "INSERT INTO `tbl_lupu_orders` (OrderID, Sal, FName, LName, Address,ZipCode,Dest_ZipCode, Phone, Phone2, EMail, Transport, SameState, Or_City, Or_State, Or_Pack, Or_Load, Dest_State, Dest_City, Dest_Unpack, Dest_Unload, Charged, Labor, CardType, CardNo, IP, Domain, MoveType, MoveDate, OrderTime, CustomerID)
		VALUES('$LeadId', '$salut', '$fname', '$lname', '$address', '$zipcode', '$dest_zipcode',$phone1, $phone2, '$email', '$transport', '$samecity', '$or_city','$or_state', '$or_pack', '$or_load', '$dor_state', '$dor_city', '$dor_pack', '$dor_load', '0', $labors, '$card','$credit', '$IP', '$user_domain', '$serv1', '$st_date', CURRENT_TIMESTAMP, '$customer_id')";
...
$query = "INSERT INTO `tbl_transport_orders` (OrderID, Sal, FName, LName, Address,ZipCode, Dest_ZipCode, Phone, Phone2, EMail , MoveType, Or_City, Or_State,  Dest_State,   IP, Domain,  MoveDate, OrderTime)
		VALUES('$LeadId', '$salut', '$fname', '$lname', '$address', '$zipcode', '$dest_zipcode',$phone1, $phone2, '$email', '$serv1', '$or_city','$or_state', '$dor_state', '$IP', '$user_domain',  '$st_date', CURRENT_TIMESTAMP)";
...
$dor_zip_database="";
    if($or_country == "USA")
    {
               $zip_sql ="SELECT `lat`, `lon` FROM zip_usa WHERE zipcode='$zipcode' LIMIT 1";
               $r_zip = mysql_query($zip_sql) or die('we could not retrieve you zipcode loaction');
               if(mysql_num_rows($r_zip) ==1){
               list($or_lat, $or_long) = mysql_fetch_array($r_zip);}
               $or_zip_database="zip_usa";
    }else{
                $zip_sql ="SELECT `lat`, `lon` FROM zip_canada WHERE zipcode='$zipcode' LIMIT 1";
               $r_zip = mysql_query($zip_sql) or die('we could not retrieve you zipcode loaction');
               if(mysql_num_rows($r_zip) ==1){
               list($or_lat, $or_long) = mysql_fetch_array($r_zip);}
               $or_zip_database="zip_canada";
    }

        if($dor_country == "USA")
        {
                   $zip_sql ="SELECT `lat`, `lon` FROM zip_usa WHERE zipcode='$dest_zipcode' LIMIT 1";
                   $r_zip = mysql_query($zip_sql) or die('we could not retrieve you zipcode loaction');
               if(mysql_num_rows($r_zip) ==1){
                   list($dor_lat, $dor_long) = mysql_fetch_array($r_zip);}
               $dor_zip_database="zip_usa";
        }else{
                    $zip_sql ="SELECT `lat`, `lon` FROM zip_canada WHERE zipcode='$dest_zipcode' LIMIT 1";
               $r_zip = mysql_query($zip_sql) or die('we could not retrieve you zipcode loaction');
               if(mysql_num_rows($r_zip) ==1){
               list($dor_lat, $dor_long) = mysql_fetch_array($r_zip);}
               $dor_zip_database="zip_canada";
        }
...
if ($transport == "yes")
{
	if($serv1=="0")
	{ 
		$queryTrans = " OR (tblmembers.MemberType = 'transport' AND tblmembers.Active = '1' AND (State = '$or_shname' OR (State = 'NA' AND ServiceCountry='$or_country')) AND InterstateLicense !='1' )";

	}
	else{
		$queryTrans = " OR (tblmembers.MemberType = 'transport'  AND tblmembers.Active = '1' AND InterstateLicense='1' AND ServiceCountry='$or_country')";
	}
...
Select 
	...
	IFNULL(ROUND(DEGREES(ACOS(SIN(RADIANS($or_lat))*SIN(RADIANS(lat))+COS(RADIANS($or_lat))*COS(RADIANS(lat))*COS(RADIANS($or_long-lon)))),2)* 69.09,0) AS distance
  
From 
	tblmembers LEFT JOIN $or_zip_database ON $or_zip_database.zipcode =tblmembers.ZipCode
				     
Where 
	(
		(
			tblmembers.MemberType = 'standard' OR tblmembers.MemberType = 'full' 
		) AND tblmembers.Active = '1' AND
		(
			(

				
				tblmembers.State = '$or_shname' AND 	
				IFNULL(ROUND(DEGREES(ACOS(SIN(RADIANS($or_lat))*SIN(RADIANS(lat))+COS(RADIANS($or_lat))*COS(RADIANS(lat))*COS(RADIANS($or_long-lon)))),2)* 69.09,0) < tblmembers.distance
			) OR  	
			(
				tblmembers.State = 'NA' AND 
				tblmembers.MemberState = '999' AND 
				tblmembers.ServiceCountry = '$or_country'
			) OR 
			(
				tblmembers.State = 'NA' AND
				tblmembers.ServiceCountry = '$or_country' AND 
						IFNULL(ROUND(DEGREES(ACOS(SIN(RADIANS($or_lat))*SIN(RADIANS(lat))+COS(RADIANS($or_lat))*COS(RADIANS(lat))*COS(RADIANS($or_long-lon)))),2)* 69.09,0) < tblmembers.distance
...
FROM
	tblmembers LEFT JOIN $dor_zip_database ON $dor_zip_database.zipcode =tblmembers.ZipCode
...
Where 
	(
		...
		(
			(
				tblmembers.State = '$dor_shname' AND 
...
".$queryTrans."";
	...
	store_orders($or_ret, $dor_ret, $LeadId, $valid);
			      ...
			      $sms_address = $val[6];		
			          ...
			          $message= transport_message($CName, $origin, $samecity, $dest, $phone1, $phone2, $st_date, $email);
                                 ...
                                 $Subject = "New Order from MMWC";
                                             ...
                                             $message=member_origin($LeadId, $or_service, $origin, $st_date, $labors, $val[7]);
		...
		foreach($dor_ret as $val)
			{
			      ...
			      $sms_address = $val[6];			
			          ...
			          $message= transport_message($CName, $origin, $samecity, $dest, $phone1, $phone2, $st_date, $email);
                                ...
                                else if(($or_none==1) && $memberType !='transport'){
                                        $Subject = "New Order from MMWC";
                                        $message=member_destination($LeadId, $dor_service, $dest, $st_date, $labors, $val[7]);
                                }
			...
			$message = nl2br($message);
                        ...
                        if(($sms_service=="yes" && $sms_address !="") &&( $or_none==1))
                        {
                        send_or_store($valid, $AdminMail, $sms_address, "New Order", $sms_message, $LeadId, $IP);                          
                        }
