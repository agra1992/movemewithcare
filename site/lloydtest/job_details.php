...
if($repost != 1 && $accept != 1)
{
  ...
  $str = "Select tbl_lupu_orders.Sal, tbl_lupu_orders.FName, tbl_lupu_orders.LName, tbl_lupu_orders.Address, tbl_lupu_orders.$zip_query,
...
IFNULL(ROUND(DEGREES(ACOS(SIN(RADIANS($lat))*SIN(RADIANS(lat))+COS(RADIANS($lat))*COS(RADIANS(lat))*COS(RADIANS($lon-lon)))),2)* 69.09,0) AS distance
           From tbl_lupu_orders LEFT JOIN $zip_database ON($zip_database.zipcode=tbl_lupu_orders.$zip_query) Where tbl_lupu_orders.OrderID = $OrderID";
