...
if($store1=="yes")
{
	...
	$st_date = $Year1 . "-" . $Month1 . "-" . $Day1;
	$st_date2 = $Year2 . "-" . $Month2 . "-" . $Day2;
	...
	$sql = "SELECT COUNT(*) as numorders from tbl_storage_orders WHERE IP = '$IP' AND OrderTime LIKE '$datevar%'";
		...
		{
		  $query = "INSERT INTO `tbl_storage_orders` (OrderID, Sal, FName, LName, Address, ZipCode, Phone, Phone2, EMail, Services, 
		         Or_State, Or_City, Dest_State, IP, Domain , MoveType, SDate, EDate, StorageSize, OrderTime)
           VALUES ('$LeadId', '$salute', '$fname', '$lname', '$address', '$zip', CONCAT('$ph1','$ph2','$ph3'), 
					CONCAT('$ph4','$ph5','$ph6'), '$email', '$services', '$o_state', '$city', '$d_state',
					'$IP', '$user_domain', '$serv1','$st_date', '$st_date2', '$st_size', CURRENT_TIMESTAMP)";
...
if($store1=="yes")
{
  ...
  if((in_array("2",$serv)) || (in_array("1",$serv)) || (in_array("4",$serv)))
	{
	$showrec="lupu";
	}
...
//echo $query10;	 
	 $result10 = mysql_query($query10) or die('Query failed: 10' . mysql_error());
	...
	{
		$sql = "INSERT INTO `tbljobs_members_storage` (`JID`, `MID`) VALUES ('$LeadId', '$line[MemberID]')";
		$result = mysql_query($sql) or die("Query failed5***");
	}
