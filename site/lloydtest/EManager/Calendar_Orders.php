    	...
    	while ($d <= $daysInMonth)
    	{
    	    ...
    	    for ($i = 0; $i < 7; $i++)
    	    {
				...
				/*    if ((strlen($d) == "1") && ($d != "0"))
					...
					$str = "Select tbljobs_members.JMID From tblorders_jobs
                                 Inner Join tbljobs_members ON tblorders_jobs.JobID = tbljobs_members.JID
                                   Where
                                    tblorders_jobs.OrderType = 'LUPU' AND
                                  tblorders_jobs.`Date` Like '$DATE_VAR%' AND tbljobs_members.`MID` =" . $_SESSION['Member_Id'];
					//echo $str;
