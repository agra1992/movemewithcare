...
class Calendar
{
    ...
    */
    function getMonthView($month, $year, $state_id)
    {
        return $this->getMonthHTML($month, $year, $state_id);
    }
    ...
    */
    function getMonthHTML($m, $y, $state_id, $showYear = 1)
    {
        $s = "";
    	...
    	while ($d <= $daysInMonth)
    	{
    	    ...
    	    for ($i = 0; $i < 7; $i++)
    	    {
    	        ...
    	        if ($d > 0 && $d <= $daysInMonth)
    	        {
					...
					$str = "SELECT * FROM tbl_fs_orders WHERE tbl_fs_orders.Or_State = '".$state_id."' AND tbl_fs_orders.MoveDate = '".$DATE_VAR."%' AND tbl_fs_orders.Status != 'C'";
					$result_str = mysql_query($str) or die("Query failed_Cal");
					$num_str = mysql_num_rows($result_str);
					
					$str1 = "SELECT * FROM tbl_lupu_orders WHERE ( (tbl_lupu_orders.Or_State = '".$state_id."' AND (tbl_lupu_orders.Or_Load = '1' or tbl_lupu_orders.Or_Pack = '1')) or (tbl_lupu_orders.Dest_State = '".$state_id."' AND (tbl_lupu_orders.Dest_Unload = '1' or tbl_lupu_orders.Dest_Unpack = '1')) ) AND tbl_lupu_orders.MoveDate = '".$DATE_VAR."%' AND tbl_lupu_orders.Status != 'C'";
					$result_str1 = mysql_query($str1) or die("Query failed_Cal");
					$num_str1 = mysql_num_rows($result_str1);
					
					if (!(empty($num_str)) || !(empty($num_str1)))
					 {
						$flag = "F";
					 }
					else
					...
					if($flag == "F")
					{
					  $s .= (($link == "") ? $d : "<a href=\"$link\"><b><font color=\"red\">$d</font></b></a>");
					}
					else
					{
