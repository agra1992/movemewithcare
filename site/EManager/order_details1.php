...
?>
   ...
   <a href="orders.php?ID=<?=$ID?>&SearchString=<?=$SearchString?>&count=<?=$count?>&offset=<?=$offset?>&OrderType=<?=$OrderType?>&day=<?=$day?>&month=<?=$month?>&year=<?=$year?>&Cal=<?=$Cal?>&mod_fs=<?=$mod_fs?>">Manage Orders</a> > Order Details for Order # <? echo "$ID"; ?></div>
	<br><br>
...
<?
   ...
   if($OrderType == "LUPU")
	{
      echo "<h2>Order Details for Order # $ID&nbsp; <a href=\"javascript:spWindowOpen($ID);\" 
	...
	if($OrderType == "FS")
	{
      echo "<h2>Order Details for Order # $ID&nbsp; <a href=\"javascript:spWindowOpen1($ID);\" 
            title=\"Click here to print this page\">
		 <img src=\"graphics/print.gif\" border=\"0\"></a></h3> 
          <br><br>";
	}
...
<script language="JavaScript">
  ...
  {  
	SpdWindowOpen=window.open('print_page.php?PageType=LUPUOD&ID='+ID,'newwSp','status=yes,scrollbars=yes,width=600,height=600,left=10,top=20')
  }
  ...
  {  
	SpdWindowOpen1=window.open('print_page.php?PageType=FSOD&ID='+ID,'newwSp','status=yes,scrollbars=yes,width=600,height=600,left=10,top=20')
  }
...
<?
    if($OrderType == "LUPU")
	{
    		$str = "Select tbl_lupu_orders.OrderID, tbl_lupu_orders.Sal, tbl_lupu_orders.FName, tbl_lupu_orders.LName, 
			       tbl_lupu_orders.Address,
            ...
            tbl_lupu_orders.CardNo, tbl_lupu_orders.Phone2,tbl_lupu_orders.Origin_MID, tbl_lupu_orders.Dest_MID, tbl_lupu_orders.Status_Order From
            tbl_lupu_orders
	   ...
	   foreach($ret as $val)
		{
		  ...
		  $Phone2 = $val[27];
                  ...
                  $Status_Order= $val[30];
                                                               ...
                                                               $OriginMember="None Required";}
                                                               else if($OLoad != "" || $OPack!=""){
                                                               $OriginMember="Pending";
//also disable charge button
$disable_charge="disabled";
                                                               }
                                                               if($DestMID !=""){
                                                               ...
                                                               else if($DUnload == "" && $DUnack==""){
                                                               $DestMember="None Required";}
                                                               else if($DUnload != "" || $DUnpack!=""){
                                                               $DestMember="Pending";
//also disable charge button
$disable_charge="disabled";
                                                                }
  ...
  else
  {
	...
	$DestState = $line[name];
		  ...
		  echo "<tr>
				...
				<tr>
					<td align=\"right\" valign=\"top\"><b>Origin Mover</b></td>
					<td>$OriginMember</td>
				</tr>
				<tr>
					<td align=\"right\" valign=\"top\"><b>Destination Mover</b></td>
					<td>$DestMember</td>
				</tr>
				<tr>
					...
					if($Charged == "0")
					{
					  ...
					  echo "<b><font face=\"Verdana\" size=\"2\" color=\"red\">NOT CHARGED</font></b>
					         <form action=\"cu_order1.php\" name=\"form1\"  id=\"form1\" method=\"post\">
							 ...
							 <input type=\"hidden\" name=\"mod_fs\" value=\"$mod_fs\">
							 <input type=\"hidden\" name=\"day\" value=\"$day\">
							 <input type=\"hidden\" name=\"month\" value=\"$month\">
							 <input type=\"hidden\" name=\"year\" value=\"$year\">
							 <input type=\"hidden\" name=\"Cal\" value=\"$Cal\">

							 <input type=\"submit\" name=\"Charge\"  id=\"Charge\" value=\"Charge\" class=\"waButton1\" $disable_charge>
...
echo"</tr><tr><td align=\"right\" valign=\"top\"><b>Order Status: </b></td><td>";
                                        if($Status_Order== "G")
					{
					  echo "<b><font face=\"Verdana\" size=\"2\" >Not canceled</font></b>
					         <form action=\"cancel_order.php\" name=\"form2\" method=\"post\">
							 <input type=\"hidden\" name=\"ID\" value=\"$ID\">
							  <input type=\"hidden\" name=\"OID\" value=\"$OID\">
							  <input type=\"hidden\" name=\"OrderType\" value=\"$OrderType\">
							 <input type=\"hidden\" name=\"nSearchCrit\" value=\"$nSearchCrit\">
							 ...
							 <input type=\"hidden\" name=\"year\" value=\"$year\">
							 <input type=\"hidden\" name=\"Cal\" value=\"$Cal\">
			                                 <input type=\"hidden\" name=\"Charged\" value=\"$Charged\">
							 <input type=\"submit\" name=\"Cancel\" value=\"Cancel Order\" class=\"waButton1\">
							 </form>
                                                         <form action=\"complete_order.php\" name=\"form3\" method=\"post\">
<input type=\"hidden\" name=\"ID\" value=\"$ID\">
							  <input type=\"hidden\" name=\"OID\" value=\"$OID\">
							  <input type=\"hidden\" name=\"OrderType\" value=\"$OrderType\">
							 <input type=\"hidden\" name=\"nSearchCrit\" value=\"$nSearchCrit\">
							 ...
							 <input type=\"hidden\" name=\"year\" value=\"$year\">
							 <input type=\"hidden\" name=\"Cal\" value=\"$Cal\">
                                                          <input type=\"submit\" name=\"Complete\" value=\"Complete this Order\" class=\"waButton1\">
                                                          </form>";
					}
					else if($Status_Order=="B")
					{
					  echo "<b><font face=\"Verdana\" size=\"2\" color=\"red\">This Order Has Been Canceled</font></b>";
					          
					}
					else if($Status_Order=="C")
					{
					  echo "<b><font face=\"Verdana\" size=\"2\" color=\"green\">This Order Has Been Completed</font></b>";
				...
				<tr>
					<td align=\"right\" valign=\"top\"><b>Email Log:</b></td>
					<td>
					          <form action=\"email_log.php\" name=\"form1\" method=\"post\">
							
							  <input type=\"hidden\" name=\"OID\" value=\"$OID\">
						
							 <input type=\"submit\" name=\"Log\" value=\"Email Log\" class=\"waButton1\">
							 </form></td>
				</tr>";
  ...
  echo "<input type=button value=\"Go Back\" class=\"waButton1\" onclick=\"window.location='orders.php?ID=$ID&SearchString=$SearchString&count=$count&offset=$offset&OrderType=$OrderType&day=$day&month=$month&year=$year&Cal=$Cal&mod_fs=$mod_fs'\">";
?>
