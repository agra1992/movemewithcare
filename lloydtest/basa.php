  ...
  function Leads_Full($day,$month,$year)
  {
	...
	if ((strlen($day) != "1") && ($day != "0"))
	{
	 ...
	 $state_id=$StateInfo->StateID;
	             ...
	             $str = "SELECT * FROM tbl_fs_orders JOIN leads_sent ON leads_sent.lead_id = tbl_fs_orders.OrderID WHERE leads_sent.member_id ='".$_SESSION['Member_Id']."' AND tbl_fs_orders.MoveDate = '".$datevar."%'";
   

    $result = mysql_query($str) or die("Query failed1 $str");
  ...
  { 
	...
	if ((strlen($day) != "1") && ($day != "0"))
	{
	 ...
	 $str = "";
		...
		$str = "SELECT * FROM tbl_lupu_orders JOIN orders_sent ON orders_sent.order_id= tbl_lupu_orders.OrderID WHERE orders_sent.valid='yes' AND orders_sent.member_id ='".$_SESSION['Member_Id']."' AND tbl_lupu_orders.MoveDate = '$datevar%'";
	...
	while( $JobInfo = mysql_fetch_object($result) )
		 {
		   ...
		   if(($JobInfo->Or_Load == '1' || $JobInfo->Or_Pack == '1') && 
                       $JobInfo->Status_Origin != "C" && ($state_id == "999" || $JobInfo->Or_State == $state_id) && $JobInfo->location=='origin')
		   {
		   	   $status = "";
		   	   $repost = 0;
		   	   $accept = 0;
			   ...
			   else
			   {
			   	...
			   	$result1 = mysql_query($str1) or die("Query failed1 $str1");
		   ...
		   if(($JobInfo->Dest_Unload == '1' || $JobInfo->Dest_Unpack == '1') && 
                       $JobInfo->Status_Dest != "C" && ($state_id == "999" || $JobInfo->Dest_State == $state_id) && $JobInfo->location=='destination')
		   {
		   	   $status = "";
		   	   $repost = 0;
		   	   $accept = 0;
