...
function FS_Show()
                {
                        window.location.href = 'members_fs.php';
                }
   ...
   function Navigation()
		{
		   ...
		   if ($count > 0)
		   {
			   ...
			   if (($start+1) > 1) 
			   {
			     echo "<a href='members_recent.php?nSearchCrit=$nSearchCrit&SearchString=$SearchString&count=$count&offset=$previous'> Previous </a>";
		       }		 
			   if($count%$pagesize)
			   {
			   ...
			   for( $j=0,$k=1 ; $j<$count && $k<=$looplimit;$j+=$pagesize, $k++)
			   {
				  ...
				  {
						echo "<a href='members_recent.php?nSearchCrit=$nSearchCrit&SearchString=$SearchString&count=$count&offset=$j'> $k </a>&nbsp";
				   }		
			   }
			   if($count>($start+$pagesize))
			   {
				  $m = $start+$pagesize;
				  echo "<a href='members_recent.php?nSearchCrit=$nSearchCrit&SearchString=$SearchString&count=$count&offset=$m'> Next </a>";
			   }
			   echo "<br></center>";
   ...
   function AlphaIndex()
		{
		  ...
		  for($i=65;$i<91;$i++)
		  {
			  ...
			  {
			    echo "<a href='members_recent.php?SearchString=$strAlpha&nSearchCrit=1' title='$strAlpha'>
				          <b>$strAlpha</b> &nbsp;&nbsp;
				      </a>";  
			  }
		  }
		   ...
		   if (!isset($offset))
			...
			$date_week_ago=date('Y-m-d h:i:s', strtotime("-1 week"));
			    if (($nSearchCrit == '0') && ($Mod == "1"))
				{
					  if (!isset($count))
						  {
							$strQuery = "SELECT MemberID,MemberName,Active FROM tblmembers where ContactEmail LIKE '%$SearchString%' and DateAdded > '$date_week_ago' ORDER BY DateAdded DESC";
							$DataBase->query($strQuery);
							$count=$DataBase->get_num_rows();
						  }
						   $strQuery = "SELECT MemberID,MemberName,Active FROM tblmembers where ContactEmail LIKE '%$SearchString%' and DateAdded > '$date_week_ago' ORDER BY DateAdded DESC LIMIT $LowerLimit,$UpperLimit";
                }
				...
				{	  
					     if (!isset($count))
						  {
							$strQuery = "SELECT MemberID,MemberName,Active FROM tblmembers where MemberName LIKE '%$SearchString%' and DateAdded > '$date_week_ago' ORDER BY DateAdded DESC"; 
							$DataBase->query($strQuery);
							$count=$DataBase->get_num_rows();
						  }
						   $strQuery = "SELECT MemberID,MemberName,Active FROM tblmembers where MemberName LIKE '%$SearchString%' and DateAdded > '$date_week_ago' ORDER BY DateAdded DESC LIMIT $LowerLimit,$UpperLimit";
				}
				if (($nSearchCrit == '0') && ($Mod == "3"))
				{
					      if (!isset($count))
						  {
							$strQuery = "SELECT MemberID,MemberName,Active FROM tblmembers where ContactPerson LIKE '%$SearchString%' and DateAdded > '$date_week_ago' ORDER BY DateAdded DESC";
							$DataBase->query($strQuery);
							$count=$DataBase->get_num_rows();
						   }
						   $strQuery = "SELECT MemberID,MemberName,Active FROM tblmembers where ContactPerson LIKE '%$SearchString%' and DateAdded > '$date_week_ago' ORDER BY DateAdded DESC LIMIT $LowerLimit,$UpperLimit";
				}
				...
				if (($nSearchCrit == '0') && (!(isset($Mod))))
				{
				     if (!isset($count))
					  {
						$strQuery = "SELECT MemberID,MemberName,Active FROM tblmembers where DateAdded > '$date_week_ago' ORDER BY DateAdded DESC";
						$DataBase->query($strQuery);
						$count=$DataBase->get_num_rows();
					  }
    			       else
					  {
					   $strQuery = "SELECT MemberID,MemberName,Active FROM tblmembers where DateAdded > '$date_week_ago' ORDER BY DateAdded DESC LIMIT $LowerLimit,$UpperLimit";
					  }
				 }
...
function CheckForm(pinForm) {
   ...
   <div align="left"><a href="EManager.php">EManager(Home)</a> > <a href="members.php">Manage Members</a> > Manage Recent Members</div>
	<br><br>
	
   <table border="0" cellspacing="0" cellpadding="0" >
	  <form action="member_search_action_recent.php" method="post" onSubmit="return CheckForm(this);">
	  <input type="hidden" name="nSearchCrit" value="0">
        ...
        <tr>
           ...
           <p>
			...
			<p>
              <input name="submit3" type="button" class="waButton1" value="Show Full Service Members" onClick="FS_Show();">
            </p>
</td>
	    </tr>
	  	</form>
