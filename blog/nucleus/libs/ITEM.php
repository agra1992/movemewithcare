...
function send_all()
{
    ...
    while($result_e = mysql_fetch_row($r))
    {
    $AdminMail=$result_e[0];
    }
    $sql="SELECT mname, memail FROM nucleus_member WHERE mailinglist='1'";
    $r=mysql_query($sql) or die("Query failed6 ");

    while($result = mysql_fetch_row($r))
    {
        $message="Dear ".$result[0]."";
        $memberEmail=$result[1];
        $message='Our accredited members has just posted a new moving article that might peek your 
interest. Please visit the site <a href="www.movemewithcare.com/blog"> www.movemewithcare.com/blog</a> to view the article. Do
not hesitate to comment it or ask questions. Thank you for making movemewithcare
the most visited and used site in the internet.';
        send_mail($AdminMail, $AdminMail, $memberEmail,"New Moving Article",$message);
		$sql = "INSERT INTO `tblEmails` (`MailType`, `From`, `To`, `Subject`, `Message`, `OrderID`) VALUES ('29', '$AdminMail', '$memberEmail', 'New Moving Article', '$message', '99999')";
		$result = mysql_query($sql) or die("Query failed5 $sql");
    }
}
...
class ITEM {
	...
	function createFromRequest() {
		...
		$itemid = $blog->additem($i_catid, $i_title,$i_body,$i_more,$i_blogid,$i_author,$posttime,$i_closed,$i_draft,$posted);
                ...
                //send email to all who prescribed
                send_all();
