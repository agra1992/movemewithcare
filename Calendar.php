...
class Calendar
{
    ...
    */
    function getMonthView($month, $year, $state_id, $member_state, $service_state, $license , $country)
    {

        return $this->getMonthHTML($month, $year, $state_id,1, $member_state, $service_state, $license, $country);
    }
    ...
    */
    ...
    function getMonthHTML($m, $y, $state_id,  $showYear = 1,$member_state, $service_state, $license, $country)
    {
        $s = "";
    	...
    	$s .= "<table class=\"calendar\" border=\"1\" bordercolor=\"003366\" cellspacing=\"1\" width=\"100%\">\n";
    	$s .= "<tr>\n";
    	...
    	while ($d <= $daysInMonth)
    	{
    	    ...
    	    for ($i = 0; $i < 7; $i++)
    	    {
    	        ...
    	        if ($d > 0 && $d <= $daysInMonth)
    	        {
					...
					$DATE_VAR = $year . "-" . $VarMonth . "-" . $dayvar;

         if($member_state == "999")
	 {
		$str_lupu = "SELECT * FROM tbl_lupu_orders WHERE ( ( (tbl_lupu_orders.Or_Load = '1' or tbl_lupu_orders.Or_Pack = '1') AND tbl_lupu_orders.Status_Origin != 'C') or ( (tbl_lupu_orders.Dest_Unload = '1' or tbl_lupu_orders.Dest_Unpack = '1') AND tbl_lupu_orders.Status_Dest != 'C')) AND tbl_lupu_orders.MoveDate = '$DATE_VAR%'";
	 }
	 else 
	 {
	 	$str_lupu = "SELECT * FROM tbl_lupu_orders
                        LEFT JOIN tblcustomers on (tbl_lupu_orders.IP = tblcustomers.IP)
                         WHERE tblcustomers.valid='yes' AND                                           ( (tbl_lupu_orders.Or_State = '$member_state' AND 
                                          ...
                                          tbl_lupu_orders.MoveDate = '$DATE_VAR%'";
             ...
             if($member_state== 999)
             {
                 if($country == "USA")
                 {  
	             $str_full = "SELECT * FROM tbl_fs_orders WHERE (tbl_fs_orders.Or_State <52) AND tbl_fs_orders.MoveDate = '$DATE_VAR%'";
                  }
                  else{
 	              $str_full = "SELECT * FROM tbl_fs_orders WHERE (tbl_fs_orders.Or_State >52) AND tbl_fs_orders.MoveDate ='$DATE_VAR%'";
                  }
                 ...
                 if($license == 1)
                 {
                     if(  $country == "USA")
                     {   
	                 $str_full = "SELECT * FROM tbl_fs_orders WHERE tbl_fs_orders.Or_State <52 AND tbl_fs_orders.Or_State = '$member_state' AND tbl_fs_orders.MoveDate = '".$DATE_VAR."%'";
                     }
                     else{
	                 $str_full = "SELECT * FROM tbl_fs_orders WHERE tbl_fs_orders.Or_State >52 AND tbl_fs_orders.Or_State = '$member_state' AND tbl_fs_orders.MoveDate = '".$DATE_VAR."%'";
                      }
	             ...
	             $str_full = "SELECT * FROM tbl_fs_orders WHERE (tbl_fs_orders.Or_State = '$state_id' and tbl_fs_orders.Dest_State = '$state_id') AND tbl_fs_orders.MoveDate = '$DATE_VAR%'";
                 }
             }
					...
					{
						$result_str = mysql_query($str_lupu) or die("Query failed_Cal1");
						$num_str = mysql_num_rows($result_str);
					}
					if ($_SESSION['Member_Type'] == "full")
					{
						$result_str = mysql_query($str_lupu) or die("Query failed_Cal2");
						$num_str += mysql_num_rows($result_str);
						$result_str1 = mysql_query($str_full) or die("Query failed_Cal3");
						$num_leads = mysql_num_rows($result_str1);
						$num_str += $num_leads;
					}
					...
					if($flag == "F")
					{
						if ($num_leads == 0)
						{
					  		$s .= (($link == "") ? $d : "<a href=\"$link\"><b><font color=\"blue\">$d</font></b></a>");
						}
						else 
						{
							$s .= (($link == "") ? $d : "<a href=\"$link\"><b><font color=\"green\">$d</font></b></a>");
						}
					}
					else
					{
					  $s .= (($link == "") ? $d : "<a href=\"$link\"><b>$d</b></a>");
					}
				}
    	        else
    	        {
    ...
    */
    function getYearHTML($year)
    {
        ...
        $s .= "<table class=\"calendar\" border=\"0\" bordercolor=\"003366\">\n";
        $s .= "<tr>";
